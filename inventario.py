...
class delInventario:
    def __init__(self):
        ...
        self.treeInventario = ttk.Treeview(self.root, columns=("Producto", "Cantidad", "Precio"), show="headings")
    ...
    def pantallaPrincipal(self):
        ...
        self.treeInventario.place(relx=0.5, rely=0.5, width=1000, height=400, anchor=tk.CENTER)
    ...
    def cargarProductos(self):
        ...
        try:
            self.cursor.execute("SELECT nombre_producto, cantidad_disponible, precio_unitario FROM inventarioproductos")
    ...
    def nuevoProducto(self):
        ...
        guardar_button = tk.Button(nuevo_producto_window, text="Guardar", command=lambda: self.guardarNuevoProducto(
            codigo_entry.get(), nombre_entry.get(), cantidad_entry.get(), precio_entry.get(), nuevo_producto_window))
        guardar_button.pack()
    ...
    def modificarProducto(self):
        ...
        nombre = values[0] if values and len(values) > 0 else ""
        cantidad = values[1] if values and len(values) > 1 else ""
        precio = values[2] if values and len(values) > 2 else ""  # Cambiado de [2] a [3]

        # Consultar el código del producto basándose en el nombre
        try:
            self.cursor.execute("SELECT id_producto FROM inventarioproductos WHERE nombre_producto = %s", (nombre,))
        ...
        guardar_button = tk.Button(modificar_producto_window, text="Guardar",
                                   command=lambda: self.guardarProductoModificado(
                                       selected_item, codigo_entry.get(), nombre_entry.get(), cantidad_entry.get(),
                                       precio_entry.get(), modificar_producto_window))
        guardar_button.pack()

    def guardarProductoModificado(self, selected_item, codigo, nombre, cantidad, precio, window):
        try:
            # Verificar si el nuevo nombre ya existe para otro producto
            self.cursor.execute(
                "SELECT COUNT(*) FROM inventarioproductos WHERE nombre_producto = %s AND id_producto != %s",
                (nombre, codigo))
            ...
            if producto_existente > 0:
                ...
                self.cursor.execute(
                    "UPDATE inventarioproductos SET nombre_producto = %s, cantidad_disponible = %s, precio_unitario = %s WHERE id_producto = %s",
                    (nombre, cantidad, precio, codigo))
                self.connection.commit()

                # Imprimir la información (puedes eliminar esto en la versión final)
                print(
                    f"Producto modificado: Código: {codigo}, Nombre: {nombre}, Cantidad: {cantidad}, Precio: {precio}")

                # Actualizar el Treeview con la información modificada
                self.treeInventario.item(selected_item, values=(codigo, nombre, cantidad, precio))
    ...
    def guardarNuevoProducto(self, codigo, nombre, cantidad, precio, window):
        try:
            # Verificar si el producto ya existe en la base de datos
            #se verifica el id
            self.cursor.execute("SELECT COUNT(*) FROM inventarioproductos WHERE id_producto = %s", (codigo,))
            producto_existente = self.cursor.fetchone()[0]
            ...
            if producto_existente > 0:
                ...
                self.cursor.execute(
                    "INSERT INTO inventarioproductos (nombre_producto, cantidad_disponible, precio_unitario) VALUES (%s, %s, %s)",
                    (nombre, cantidad, precio))
                self.connection.commit()

                # Imprimir la información (puedes eliminar esto en la versión final)
                print(f"Producto añadido: Código: {codigo}, Nombre: {nombre}, Cantidad: {cantidad}, Precio: {precio}")

                # Actualizar el Treeview con el nuevo producto
                self.treeInventario.insert("", "end", values=(codigo, nombre, cantidad, precio))
